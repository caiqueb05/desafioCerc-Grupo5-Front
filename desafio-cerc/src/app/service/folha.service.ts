...
export class FolhaService {
  ...
  getAllFolhas(): Observable<FolhaDePagamentoDTO[]>{
    return this.http.get<FolhaDePagamentoDTO[]>('http://localhost:8080/api/v1/folha/todas', this.token)
  }

  postFolha(folha: FolhaDePagamentoDTO): Observable<FolhaDePagamentoDTO>{
    return this.http.post<FolhaDePagamentoDTO>('http://localhost:8080/api/v1/folha', folha, this.token)
  }

  /* getByIdFuncionario(id: number): Observable<FolhaDePagamentoDTO>{
    return this.http.get<FolhaDePagamentoDTO>(`http://localhost:8080/api/v1/folha/${id}`, this.token)
  } */
